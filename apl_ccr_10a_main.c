...
#define		COUNT_MIN	20      // n
#define		DUTY_LED	50		// % 	
	

	Led_Frequency	=(60000/COUNT_MIN);
	Led_Duty  		=( (Led_Frequency * DUTY_LED)/100 );
...
/////////////////////////////////////////////
		...
		Gmsec60000=(unsigned int)( (ZeroTimer +  (unsigned long)Gm1) % (unsigned long)Led_Frequency);

		if(Gmsec60000 >= Led_Duty){
			bBlinck=1;  // off
		}		
		else{
			bBlinck=0; // on
